...
void loop() {
  ...
  while(true){
    ...
    if(digitalRead(HEARING)){
      ...
      balance_result = balance(BODY, bodyErrors, HELMET, helmErrors);
      break;
    }
...
boolean balance(int MPU1Addr, errorVals MPU1errors,int MPU2Addr, errorVals MPU2errors){
  ...
  while(millis() - beginTest < balTestTime){  // Test is normally 20 seconds long
    ...
    if(count % 1000 || count == 1){ // Record every 20 ms or so
      ...
      if( abs(GyroZ2 - prevGyroZ2) > abs(MPU2errors.GyroErrorZ) + errorMargin*MPU2errors.stdGyroZ ){
        gyroAngleZ2 = gyroAngleZ2+ (GyroZ2 * elapsedTime) / gyroSens;
      }
      prevGyroZ2 = GyroZ2;
  
      if(head_result){
        head_result = head_calc(gyroAngleZ, gyroAngleZ2, head_thresh);
      }
      ...
      Serial.print(gyroAngleZ2); Serial.print(",");
